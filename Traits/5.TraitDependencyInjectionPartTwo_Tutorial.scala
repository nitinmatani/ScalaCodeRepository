...
object TraitDependencyInjectionPartTwo_Tutorial extends App {
  ...
  class DonutOrderService[T] {
    def createOrder(donut: T, quantity: Int, price: Double): Int = {
      println(
        s"Saving donut order to database: donut = $donut, quantity = $quantity, price = $price"
      )
  ...
  trait DonutStoreServices {
    ...
    val donutShoppingCartService = new DonutShoppingCartService(
  ...
  trait MockedDonutStoreServices extends DonutStoreServices {
    ...
    override val donutShoppingCartService: DonutShoppingCartService[String] =
      new DonutShoppingCartService[String](
        donutInventoryService,
        donutPricingService,
        donutOrderService
      )
  ...
  object MockedDonutStoreApp
      extends DonutStoreAppController
      with MockedDonutStoreServices
