...
object MaxBy_Tutorial extends App {
  ...
  println(
    s"Maximum element in sequence of case class of type Donut, ordered by price = ${donuts
      .maxBy(donut => donut.price)}"
  )
  ...
  println(
    s"Maximum element in sequence using function from Step 3 = ${donuts.maxBy(donutsMaxBy)}"
  )
