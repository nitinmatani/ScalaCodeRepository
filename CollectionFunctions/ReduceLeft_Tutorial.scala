...
object ReduceLeft_Tutorial extends App {
  ...
  println(
    s"Sum of elements from donutPrices by calling reduceLeft function explicitly= $sum1"
  )
  ...
  println(
    s"Elements of donuts sequence concatenated by passing function to the reduceLeft function = ${donuts reduceLeft concatDonutNames}"
  )
  ...
  println(
    s"Using reduceLeftOption will NOT throw any exception = ${Seq.empty[String].reduceLeftOption(_ + ", " + _)}"
  )
}
