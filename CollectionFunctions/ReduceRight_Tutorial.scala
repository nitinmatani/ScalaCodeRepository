...
object ReduceRight_Tutorial extends App {
  ...
  println(
    s"Sum of elements from donutPrices by calling reduceRight function explicitly= $sum1"
  )
  ...
  println(
    s"Elements of donuts sequence concatenated by passing function to the reduce function = ${donuts reduceRight concatDonutNames}"
  )
  ...
  println(
    s"Using reduce right option will NOT throw any exception = ${Seq.empty[String].reduceRightOption(_ + ", " + _)}"
  )
}
