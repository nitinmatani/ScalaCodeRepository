...
object TypeAlias_Tutorial extends App {
  ...
  val shoppingItem: ShoppingCartItem =
    ShoppingCartItem(Donut("Glazed Donut", 2.50), 10)
  ...
  def calculateTotal2(shoppingCartItems: Seq[ShoppingCartItem]): Double = {
    ...
    shoppingCartItems.foreach { shoppingCartItem =>
      println(
        s"ShoppingCartItem donut = ${shoppingCartItem.donut}, quantity = ${shoppingCartItem.quantity}"
      )
    }
    10 // some random total cost
  }
